	...
	if s.chatbot != nil {
		...
		if err == nil && strings.TrimSpace(reply) != "" {
			...
			return ChatResponse{
				...
				ContextSnapshot: contextSnapshot,
				Suggestions:     []string{"Run a what-if simulation", "Check tax dashboard", "Scan an SMS"},
				Explainability: fmt.Sprintf(
					"This response is generated by a large language model (Groq %s) with FINIX-specific system prompts.",
					s.chatbot.Model()),
				// Report the model actually called. This was hardcoded to
				// llama3-8b-8192, so once GROQ_MODEL was set the audit trail
				// named a model that had not been used — and, since Groq
				// decommissioned that one, a model that no longer exists.
				ModelID:             s.chatbot.Model(),
				ModelVersion:        "1.0",
